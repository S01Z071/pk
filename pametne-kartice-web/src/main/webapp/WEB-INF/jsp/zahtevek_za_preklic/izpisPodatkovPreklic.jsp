...
<div class="container" style="text-align:center;width:75%;">
    <ul id="tabs" class="nav nav-tabs" data-tabs="tabs">
        ...
        <li id="zahtevkiPreklicTab" class="active"><a href="#">Pregled zahtevkov za preklic digitalnega potrdila</a></li>  
...
<%-- STRANI --%>
<c:if test="${fn:length(zahtevkiZaPreklic)>0}">
    <div class="container" style="text-align:center;"> 
        ...
        <div style="font-size: 16px; font-weight: bold;">
            <c:choose>
                <c:when test="${pageNum!=1}">                          
                    <a class="btn" href="${pageContext.request.contextPath}/IzpisPodatkovPreklic/1"><<&nbsp;</a> 
                    <a class="btn" href="${pageContext.request.contextPath}/IzpisPodatkovPreklic/${pageNum-1}">Prejšnja&nbsp;</a>                         
                </c:when>
                <c:otherwise>
            ...
            <c:forEach var="num" begin='${zacetek}' end='${konec}' >
                <c:if test="${pageNum ne num}">                         
                    <a class="btn" href="${pageContext.request.contextPath}/IzpisPodatkovPreklic/${num}">${num}</a>                       
                </c:if>
            ...
            <c:choose>
                <c:when test="${pageNum!=izpisCount}">                       
                    <a class="btn" href="${pageContext.request.contextPath}/IzpisPodatkovPreklic/${pageNum+1}">&nbsp;Naslednja</a> 
                    <a class="btn" href="${pageContext.request.contextPath}/IzpisPodatkovPreklic/<fmt:formatNumber value="${izpisCount}" minFractionDigits="0" maxFractionDigits="1"/>">&nbsp;>></a> 
                </c:when>
                <c:otherwise>
...
<div class="container">
    ...
    <table class="table">
        ...
        <c:forEach var="zahtevekZaPreklic" items="${zahtevkiZaPreklic}"> 
            <tr style="background-color:${zahtevekZaPreklic.status.barva}">  
                <td style="text-align:left;"><strong>#${j+(pageNum-1)*prikazovNaStran}. Zahtevek: ${zahtevekZaPreklic.crtnaKoda}</strong></td>
                <td></td>
                <td style="text-align:left;"><a href="${pageContext.request.contextPath}/ZahtevekZaPreklic/podrobno/${zahtevekZaPreklic.id}" onclick="showHideDiv('loadingDiv');"><button class="btn">Podrobno</button></a>
                </td>
            </tr>
            <tr>
                <td>Ime organizacije:</td>
                <td>${zahtevekZaPreklic.imeOrganizacije}</td>
                <td></td>
            </tr>
            <tr>
