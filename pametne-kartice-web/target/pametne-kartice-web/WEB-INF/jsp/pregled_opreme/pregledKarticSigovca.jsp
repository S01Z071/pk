...
<div class="container" style="text-align: center;">
    <div class="row">
        <div class="span10">
            <h4>Pregled kartic izdanih od SIGOV-CA</h4><br><br>
        </div>
    </div>
    ...
    <div class="row">
        <div class="span6">
            <sec:authorize access="hasAnyRole('002,005,006')">
                ...
                <form method="POST" name="iskanje" action="${pageContext.request.contextPath}/AdminIskanjeKartSig/">
                    <table>
                        <tr>
...
<c:if test="${fn:length(seznamKarticSigovca)>0}">
    <table class="table">
        <tr>
            <td colspan="6">
                <strong>Skupaj: ${skupajKartic} </strong>
            </td>
        </tr>          
        <tr style="background-color:gainsboro;">    
            <td>Ime in priimek: ${seznamKarticSigovca.key}</td>                 
        ...
        <c:forEach var="seznamKarticSigovca" items="${seznamKarticSigovca}" varStatus="i">
            <c:forEach var="kartice" items="${seznamKarticSigovca.value}" varStatus="j">
                <c:choose> 
                    ...
                    <c:if test="${j.index==0}">
                        <td style="font-weight: bold;">
                            <c:if test="${fn:length(seznamKarticSigovca.value)>1}"><a id="expandCollapseSign${i.index}" onclick="expandCollapse('${i.index}');" style="cursor: pointer;">-</a></c:if> 
                            <c:if test="${fn:length(seznamKarticSigovca.value)<=1}"><a></a></c:if> 
                            ${seznamKarticSigovca.key}
                        </td>
                    </c:if>
                    <c:if test="${j.index!=0}">
                        <td></td>
                    </c:if>
                    <td>${kartice.imeOrganizacije}</td>
                    <td>${kartice.tip}</td>
                    <td>${kartice.oznaka}</td>
                    <td><fmt:formatDate value="${kartice.datumIzdaje}" pattern="dd.MM.yyyy" /></td>
                    <sec:authorize access="hasAnyRole('002,005,006')">
